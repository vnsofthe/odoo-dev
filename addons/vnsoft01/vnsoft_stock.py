    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        for i in self.browse(cr,uid,ids,context):
            ...
            if i.from_qty > product.qty_available:
                raise osv.except_osv(u"错误",u"拆包原产品数量[%d]不可以大于产品在手数量[%d]。" %(i.from_qty,i.product_from.qty_available))
            #处理拆包前的产品调整
            ...
            #处理拆包后产品调整
            amt = i.product_from.standard_price * i.from_qty
            if i.product_to.cost_method=="average":
                #调整拆包后产品的成本单价
                new_price = (i.product_to.standard_price * i.product_to.qty_available + amt) / (i.product_to.qty_available+i.to_qty)
                self.pool.get("product.product").write(cr,uid,i.product_to.id,{"standard_price":new_price})
            product = i.product_to.with_context(location=i.location_to.id)
            inventory_id = inventory_obj.create(cr, uid, {
                'name': u'拆包: %s' % tools.ustr(i.product_to.name),
                'product_id': i.product_to.id,
                'location_id': i.location_to.id}, context=context)
            th_qty = product.qty_available
            line_data = {
                'inventory_id': inventory_id,
                'product_qty': th_qty + i.to_qty,
                'location_id': i.location_to.id,
                'product_id': i.product_to.id,
                'product_uom_id': i.product_to.uom_id.id,
                'theoretical_qty': th_qty
            }
            inventory_line_obj.create(cr , uid, line_data, context=context)
            inventory_obj.action_done(cr, uid, [inventory_id], context=context)
