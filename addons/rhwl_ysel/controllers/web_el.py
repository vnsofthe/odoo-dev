...
class el(http.Controller):
    @http.route("/web/rhwl_el/images/",type="http",auth="user")
    def index(self,**kw):
        fname = os.path.join(os.path.split(os.path.split(__file__)[0])[0],"static/webcam_el.html")
        f=open(fname,"r")
        html=f.readlines()
        f.close()
        return ''.join(html)

    @http.route("/web/rhwl_el/get/",type="http",auth="user")
    def get_detail(self,**kw):
        registry = RegistryManager.get(request.session.db)
        obj = registry.get("rhwl.genes.el")
        sexdict={'M':u'男','F':u'女'}
    ...
    @http.route("/web/api/rhwl_el/pic/",type="http",auth="user")
