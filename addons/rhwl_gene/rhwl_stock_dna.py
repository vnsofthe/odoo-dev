...
class rhwl_dna(osv.osv):
    ...
    def create(self,cr,uid,val,context=None):
        ...
        gene_id = self.pool.get("rhwl.easy.genes").search(cr,uid,[("date",">=",val["start_date"]),("date","<=",val["end_date"])],context=context)
    ...
    def write(self,cr,uid,ids,val,context=None):
        ...
        for i in self.browse(cr,uid,ids,context=context):
            gene_id = self.pool.get("rhwl.easy.genes").search(cr,uid,[("date",">=",i.start_date),("date","<=",i.end_date)],context=context)
...
class rhwl_dna_line(osv.osv):
    ...
    _sql_constraints = [
        ('rhwl_gene_stock_dna_line_name_uniq', 'unique(name)', u'样本编号不能重复!'),
    ]
